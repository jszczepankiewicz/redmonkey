...
public class CachingFilterTest {
    ...
    public void notAccessedYetByTheClientButCachedServiceShouldBeReturnedFromCache() throws Exception{
        ...
        when(policyMock.getfor(request)).thenReturn(cacheableRegion());
        CacheQueryResult cachedContent = storedValueDifferentThanClient();
    ...
    private CacheQueryResult storedValueDifferentThanClient(){
        CacheQueryResult result = new CacheQueryResult(false, "{somepayload}", "assumedNewerEtag", "x");
        return result;
    }

    private CacheQueryResult storedValueSameAsOnClientSide() {
        CacheQueryResult result = new CacheQueryResult(false, null, null, "x");
        return result;
    }
